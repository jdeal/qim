...
test('select string', t => t.deepEqual(selectEach([], 'abc'), ['a', 'b', 'c']));

test('select slice of string', t =>
  t.deepEqual(selectEach([$slice(0, 2)], 'abc'), ['a', 'b']));

test('select number', t => t.throws(() => selectEach([], 5)));

test('select undefined', t => t.throws(() => selectEach([], undefined)));
...
test('update slice of string', t =>
  t.deepEqual(updateEach([$slice(0, 2)], 'abc'), 'ABc'));

test('update number', t => t.throws(() => updateEach([], 5)));

test('update undefined', t => t.throws(() => updateEach([], undefined)));
